...
func (a *AuthCommand) generateHostKeys(ctx context.Context, clusterAPI auth.ClientI) error {
	...
	key.Cert, err = clusterAPI.GenerateHostCert(key.KeyPair.PublicKeyRaw(),
		"", "", principals,
...
func (a *AuthCommand) generateDatabaseKeysForKey(ctx context.Context, clusterAPI auth.ClientI, key *client.ClientKey) error {
	...
	csr, err := tlsca.GenerateCertificateRequestPEM(subject, key.KeyPair.PrivateKeyRaw())
	if err != nil {
		return trace.Wrap(err)
	}
...
func (a *AuthCommand) generateUserKeys(ctx context.Context, clusterAPI auth.ClientI) error {
	...
	certs, err := clusterAPI.GenerateUserCerts(ctx, proto.UserCertsRequest{
		PublicKey:         key.KeyPair.PublicKeyRaw(),
		Username:          a.genUser,
		Expires:           reqExpiry,
