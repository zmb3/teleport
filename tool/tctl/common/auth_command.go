...
func (a *AuthCommand) generateWindowsCert(ctx context.Context, clusterAPI auth.ClientI) error {
	...
	certDER, _, err := windows.GenerateCredentials(ctx, a.windowsUser, a.windowsDomain, a.genTTL,
		cn.GetClusterName(), windows.LDAPConfig{Domain: a.windowsDomain}, clusterAPI)
	if err != nil {
		return trace.Wrap(err)
	}
