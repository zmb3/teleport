...
<Tabs>
  ...
  <TabItem label="Amazon Linux 2/RHEL/CentOS (RPM)">
...
Adjust the path provided to `curl` for the download option that matches your system architecture:

    ```code
    $ curl https://get.gravitational.com/teleport-ent-v(=teleport.version=)-linux-{YOUR-SYSTEM-Architecture}-bin.tar.gz.sha256
    # <checksum> <filename>
    $ curl -O https://get.gravitational.com/teleport-ent-v(=teleport.version=)-linux-{YOUR-SYSTEM-Architecture}-bin.tar.gz
    $ shasum -a 256 teleport-ent-v(=teleport.version=)-linux-{YOUR-SYSTEM-Architecture}-bin.tar.gz
    # Verify that the checksums match
    $ tar -xzf teleport-ent-v(=teleport.version=)-linux-*-bin.tar.gz
    $ cd teleport
    $ sudo ./install
    ```
  ...
  <TabItem label="deb Package">
    ...
    After downloading the `.deb` file for your system architecture, install it with `dpkg`. The example
    ...
    Preparing to unpack teleport-ent_(=teleport.version=)_amd64.deb ...
  ...
  <TabItem label="Amazon Linux 2/RHEL (RPM)">
    After downloading the `.rpm` file for your system architecture, install it with `rpm`. The example
    below assumes the `root` user and the amd64 architecture:
