...
remaining authenticated with only a single Auth Service. The leaf cluster can be
running behind a firewall without any ingress ports open.
...
In role mappings, the wildcard character `*` can be used to match any number of
