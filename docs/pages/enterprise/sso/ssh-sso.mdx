...
## Encrypted SAML SSO Assertions

Teleport 6.1 onward supports encrypted SAML assertions in responses from SAML identity providers
which adds additional security by preventing proxies and other middleman infrastructure with access
to the unencrypted request from extracting assertion information. Guides can be found on the page
for your identity provider if supported.

We currently test and officially support the following identity providers for encrypted assertions
although other providers may also work.
