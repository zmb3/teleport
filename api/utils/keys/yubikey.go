...
func getOrGenerateYubiKeyPrivateKey(ctx context.Context, serialNumber uint32, touchRequired bool) (*PrivateKey, error) {
	y, err := getYubikey(ctx, serialNumber)
	if err != nil {
...
func parseYubiKeyPrivateKeyData(keyDataBytes []byte) (*YubiKeyPrivateKey, error) {
	...
	y, err := getYubikey(ctx, keyData.SerialNumber)
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
// getYubikey gets a yubiKey PIV card by serial number.
func getYubikey(ctx context.Context, serialNumber uint32) (*yubiKey, error) {
	yubiKeyCards, err := findYubiKeyCards()
	if err != nil {
		return nil, trace.Wrap(err)
	}
