...
type Authority interface {
	...
	GenerateKeyPair(ctx context.Context, passphrase string) (privKey []byte, pubKey []byte, err error)

	// GetNewKeyPairFromPool returns new keypair from pre-generated in memory pool
	GetNewKeyPairFromPool(ctx context.Context) (privKey []byte, pubKey []byte, err error)
	...
	// a host certificate.
	GenerateHostCert(ctx context.Context, certParams services.HostCertParams) ([]byte, error)

	// GenerateUserCert generates user certificate, it takes pkey as a signing
	// private key (user certificate authority)
	GenerateUserCert(ctx context.Context, certParams services.UserCertParams) ([]byte, error)
