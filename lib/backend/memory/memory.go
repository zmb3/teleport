...
func (m *Memory) Create(ctx context.Context, i backend.Item) (*backend.Lease, error) {
	...
	defer m.Unlock()
	m.removeExpired()
	if ok := m.tree.Has(&btreeItem{Item: i}); ok {
...
func (m *Memory) Get(ctx context.Context, key []byte) (*backend.Item, error) {
	...
	defer m.Unlock()
	m.removeExpired()
	item, ok := m.tree.Get(&btreeItem{Item: backend.Item{Key: key}})
	if !ok {
...
func (m *Memory) Delete(ctx context.Context, key []byte) error {
	...
	defer m.Unlock()
	m.removeExpired()
	if ok := m.tree.Has(&btreeItem{Item: backend.Item{Key: key}}); !ok {
...
func (m *Memory) KeepAlive(ctx context.Context, lease backend.Lease, expires time.Time) error {
	...
	defer m.Unlock()
	m.removeExpired()
	i, ok := m.tree.Get(&btreeItem{Item: backend.Item{Key: lease.Key}})
	if !ok {
...
func (m *Memory) CompareAndSwap(ctx context.Context, expected backend.Item, replaceWith backend.Item) (*backend.Lease, error) {
	...
	defer m.Unlock()
	m.removeExpired()
	i, ok := m.tree.Get(&btreeItem{Item: expected})
	if !ok {
...
func (m *Memory) processEvent(event backend.Event) {
	switch event.Type {
	case types.OpPut:
		item := &btreeItem{Item: event.Item, index: -1}z
		treeItem, ok := m.tree.Get(&btreeItem{Item: event.Item, index: -1})
	...
	case types.OpDelete:
		item, ok := m.tree.Get(&btreeItem{Item: event.Item})
		if !ok {
			return
		}
