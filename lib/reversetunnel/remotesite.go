...
func (s *remoteSite) handleHeartbeat(conn *remoteConn, ch ssh.Channel, reqC <-chan *ssh.Request) {
	defer func() {
		...
		if err := conn.Close(); err != nil && !errors.Is(err, net.ErrClosed) {
			s.WithError(err).Warn("Failed to close remote connection for remote site %s", s.domainName)
		}

		if err := s.srv.onSiteTunnelClose(s.domainName, s); err != nil {
	...
	for {
		...
		if err != nil {
			...
			case trace.IsCompareFailed(err):
				...
				s.srv.onSiteTunnelClose(s.domainName, &alwaysClose{Closer: s})
				return
