...
func Write(cfg WriteConfig) (filesWritten []string, err error) {
	...
	case FormatFile:
		...
		idFile := &identityfile.IdentityFile{
			PrivateKey: cfg.Key.KeyPair.PrivateKeyRaw(),
	...
	case FormatOpenSSH:
		...
		err = writer.WriteFile(keyPath, cfg.Key.KeyPair.PrivateKeyRaw(), identityfile.FilePermissions)
		if err != nil {
			return nil, trace.Wrap(err)
		}
	...
	case FormatMongo:
		...
		err = writer.WriteFile(certPath, append(cfg.Key.TLSCert, cfg.Key.KeyPair.PrivateKeyRaw()...), identityfile.FilePermissions)
		if err != nil {
			return nil, trace.Wrap(err)
		}
		var caCerts []byte
