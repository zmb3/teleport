...
type Config struct {
	...
	// LDAPConfig is an LDAP configuration for connecting to SQL Server with x509 authentication
...
func (s *Server) startSQLServerLDAP(ctx context.Context, database types.Database) error {
	...
	s.caClients[database.GetName()] = windows.NewCertificateStoreClient(windows.CertificateStoreConfig{
		AccessPoint:   s.cfg.AuthClient,
		AuthClient:    s.cfg.AuthClient,
		LDAPConfig:    s.cfg.LDAPConfig,
		Log:           s.log,
		ClusterName:   clusterName.GetClusterName(),
		LC:            &windows.LDAPClient{Cfg: s.cfg.LDAPConfig},
		CertTTL:       sqlServerCertTTL,
